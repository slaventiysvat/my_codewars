...
//char players_win[10][4] = { "1:0","2:0","3:0","4:0","2:1","3:1","4:1","3:2","4:2","4:3" };
const char* const games[10] = { "1:0","2:0","3:0","4:0","2:1","3:1","4:1","3:2","4:2","4:3" };
int players_win_out[10] = { 0 };
int tmp_arr[2] = { 0 };
...
char** inArray(const char* arr1[], int sz1,const char* arr2[], int sz2, int* lg) {
	...
	int tmp_len2 = 0;
	//result array
	//char** rez_array = (char**)malloc(sizeof(char)*sz1);

	//for (int count = 0; count < sz1; count++) {
	//	tmp_len = strlen(arr1[count]) + 1;
	//	rez_array[count] = (char*)malloc(sizeof(char) * tmp_len); // и пять столбцов
...
int main() {
	...
	const char* arr1[] = { "cod", "code", "wars", "ewar", "pillow", "bed", "phht" };
	const char* arr2[] = { "lively", "alive", "harp", "sharp", "armstrong", "codewars", "cod", "code" };
	int sz1 = sizeof(arr1) / sizeof(*arr1);
	int sz2 = sizeof(arr2) / sizeof(*arr2);
	int lg = 0;

	char** ptr = inArray(arr1, sz1, arr2, sz2, &lg);
